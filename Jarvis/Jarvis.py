...
def smart_reply(user_input):
    ...
    reply = response[0]["generated_responses"][-1]
    speak(reply)
    print("Jarvis:", reply)
...
def screenshot():
    folder_path = r"C:\Users\leoha_gxz0s6l\Pictures"
    os.makedirs(folder_path, exist_ok=True)  # ✅ creates folder if missing

    img_path = os.path.join(folder_path, "screenshot.png")
    img = ImageGrab.grab()
    img.save(img_path)
